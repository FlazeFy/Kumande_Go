...
func UserTrackRouter(r *gin.Engine, userTrackController UserTrackController, redisClient *redis.Client, db *gorm.DB) {
	api := r.Group("/api/v1")
	{
		...
		protected_user_tracks_all.Use(middlewares.AuthMiddleware(redisClient, "user"))
		{
			protected_user_tracks_all.GET("/most_context/:target_col", userTrackController.GetMostContextUserTrack)
		}
	}
}
